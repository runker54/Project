...
def get_active_sheet(wb1):
    """
    Return the index of the active sheet.
    If the sheet set to active is hidden return the next visible sheet or None
    """
    visible_sheets = [idx for idx, sheet in enumerate(wb1._sheets) if sheet.sheet_state == "visible"]
    if not visible_sheets:
        raise IndexError("At least one sheet must be visible")

    idx = wb1._active_sheet_index
    sheet = wb1.active
...
class WorkbookWriter:
    ...
    def __init__(self, wb1):
        self.wb1 = wb1
        self.rels = RelationshipList()
        self.package = WorkbookPackage()
        self.package.workbookProtection = wb1.security
        self.package.calcPr = wb1.calculation


    def write_properties(self):

        props = WorkbookProperties() # needs a mapping to the workbook for preservation
        if self.wb1.code_name is not None:
            props.codeName = self.wb1.code_name
        if self.wb1.excel_base_date == CALENDAR_MAC_1904:
            props.date1904 = True
        self.package.workbookPr = props


    def write_worksheets(self):
        for idx, sheet in enumerate(self.wb1._sheets, 1):
    ...
    def write_refs(self):
        for link in self.wb1._external_links:
            # need to match a counter with a workbook's relations
            rId = len(self.wb1.rels) + 1
    ...
    def write_names(self):
        defined_names = copy(self.wb1.defined_names)
