...
if __name__ == '__main__':
    def tiHUan(mBan, xinXi, baoCun):
        ...
        for table_row in range(1, sheet.nrows):
            ...
            save_as(doc, "%s%s%s.docx" % (baoCun,
                                          str(sheet.cell_value(table_row, 0)), str(sheet.cell_value(table_row, 7))))
            # close(doc)
            # if table_row == sheet.nrows - 1:
            #     close(doc)


    mBan_wei = 'C:/Users/65680/Desktop/安全利用类调查表.docx'  # 模板存放位置
    xxb = 'C:/Users/65680/Desktop/大方县措施表.xls'  # 信息表存放位置
    bc = 'C:/Users/65680/Desktop/TP/'  # 文件输出位置
