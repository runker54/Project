...
import random
import time
index_text = r'C:\Users\65680\Desktop\read.txt'  # 具有页码索引的txt
picture_adress = r'C:\Users\65680\Desktop\last'  # 拆分好的图片所在的位置
mask_logo = r'C:\Users\65680\Desktop\logo.png'  # 印章所在位置
...
def calc_index(*args):
    ...
    calc_stream_list = [str(int(one_line.split(' ')[1].strip()) + 15) for one_line in calc_stream]
...
def mask_picture_logo(p1, p2, p3, p4):
    ...
    # 所有图片的地址列表
    picture_list = [os.path.join(p1, one_pictures) for one_pictures in os.listdir(p1)]
    for one_mask_pictures in picture_list:
        ...
        if index_number_str in calc_index(index_text):
            ...
            print(survey_sheet_path, instructions_path)
            # 对调查表和情况说明表盖章输出
...
mask_picture_logo(picture_adress, mask_logo, (222, 172), (977, 1402))
print(f'{"正在加盖印章"}')
time.sleep(10)
# png_to_pdf(picture_adress, pdf_name)
print(f'{"合并PDF中"}')
