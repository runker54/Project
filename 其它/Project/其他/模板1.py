...
ws.write_merge(0, 0, 0, 22, '附表1', style()[7])  # 附表
...
ws.write_merge(6, 6, 4, 15, '2公里范围内有货曾经有工矿企业、冶炼厂', style()[1])
...
cs_list_0 = ['', '', '', '', '', '', '', '']
cs_list_1 = ['种植结构调整（调整为非食用农产品）', '划定为特定农产品严格管控区', '退耕还林还草', '耕地利用变更为非耕地', '休耕', '其它措施', '', '']
cs_list_2 = ['', '', '', '', '', '', '', '']
cs_list_3 = ['', '', '', '', '', '', '', '']
for _c in range(17, 23):
    ws.write(_c, 4, '£', style()[9])  # 特殊格式
    ws.write(_c, 12, '', style()[8])  # 特殊格式
    ...
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # zz
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # hd
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # tg
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # bg
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # xg
    ws.write_merge(_c, _c, 18, 21, '', style()[8])  # qt
...
ws.write_merge(4, 4, 7, 13, '', style()[0])  # 地块编号
ws.write_merge(4, 4, 17, 22, '', style()[0])  # 地理位置
ws.write_merge(5, 5, 7, 17, '', style()[0])  # 中心坐标
ws.write_merge(5, 5, 20, 22, '', style()[0])  # 面积
ws.write(6, 16, '£', style()[2])  # 有
ws.write(6, 20, '£', style()[2])  # 无
ws.write_merge(7, 7, 8, 13, '', style()[0])  # 19年水稻主要品种
ws.write_merge(7, 7, 18, 22, '', style()[0])  # 19年玉米主要品种
ws.write_merge(8, 8, 8, 13, '', style()[0])  # 19年水稻总面积
ws.write_merge(8, 8, 18, 22, '', style()[0])  # 19年玉米总面积
...
ws.write_merge(11, 11, 8, 22, '', style()[0])  # 19年其他主要作物

ws.write_merge(7 + 5, 7 + 5, 8, 13, '', style()[0])  # 20年水稻主要品种
ws.write_merge(7 + 5, 7 + 5, 18, 22, '', style()[0])  # 20年玉米主要品种
ws.write_merge(8 + 5, 8 + 5, 8, 13, '', style()[0])  # 20年水稻总面积
ws.write_merge(8 + 5, 8 + 5, 18, 22, '', style()[0])  # 20年玉米总面积
...
ws.write_merge(11 + 5, 11 + 5, 8, 22, '', style()[0])  # 19年其他主要作物
...
workbook.save(r'C:\Users\65680\Desktop\xls2.xls')
