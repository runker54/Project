...
def su_total(data_path, output_path, so_id, index):
    ...
    with arcpy.da.SearchCursor(data,
                               ["LBBM", "DKBM", "XZQMC", "", "CBFDBXM", "DKMC", "SUM_MJ_MU",
                                "TXZQMC"]) as cursor:
        for row in cursor:
            txz_qmc = row[7]
            lb_bm = row[0]
    ...
    for ys in x_list[so_id]:  # ys 默认为村名
        ...
        for sx in pm_list:
            ...
            ws = workbook.add_sheet('%s' % ys + sx)  # Excel新增Sheet
            ...
            ws.write_merge(0, 0, 0, 15, 'mark_logo           乡（镇）农作物种植情况调查表', style_1)
            ...
            workbook.save('%s%s.xls' % (czpath, sx))

data_path = "E:/T湄潭县台账数据.gdb/T无措施数据_确权_严格管控类"  # 数据所在路径
