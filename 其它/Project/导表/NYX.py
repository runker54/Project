...
def set_excel_title(ws, xz):
    ...
    ws.write_merge(0, 0, 0, 26, '纳雍县耕地种植情况调查表（三）', style_1)
    ...
    ws.write_merge(3, 4, 13, 13, '烤烟', style_2)
    ws.write_merge(3, 4, 14, 14, '辣椒', style_2)
    ws.write_merge(3, 4, 15, 15, '蔬菜', style_2)
    ws.write_merge(3, 4, 16, 16, '皂角', style_2)
    ws.write_merge(3, 4, 17, 17, '刺梨', style_2)
    ws.write_merge(3, 4, 18, 18, '果树', style_2)
    ws.write_merge(3, 4, 19, 19, '中药材', style_2)
    ws.write_merge(3, 4, 20, 20, '茶叶', style_2)
    ws.write_merge(3, 4, 21, 21, '林木', style_2)
    ws.write_merge(3, 4, 22, 22, '其他', style_2)
...
def su_total(data_path, output_path, so_id, index):
    ...
    for ys in x_list[so_id]:  # ys 默认为村名
        ...
        ws.write_merge(len(ys_name_list) + 5, len(ys_name_list) + 5, 5, 6, xlwt.Formula('SUM(G6:G%s)' % (len(ys_name_list) + 5)), style_5)
        ...
        workbook_1.save('%s/%s.xls' % (czpath, cz))
