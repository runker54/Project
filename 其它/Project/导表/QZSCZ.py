...
data_path = "E:/Space.gdb/T无措施数据_确权_安全利用类"  # 数据所在路径
...
def set_excel_title(ws, xz):
    ...
    ws.write_merge(0, 0, 0, 26, '清镇市耕地种植情况调查表（二）', style_1)
...
def su_total(data_path, output_path, so_id, index):
    ...
    cm_list = []  # 收集所有村的列表
    #  索引地理数据库数据
    cursor = arcpy.da.SearchCursor(data, ["BSBM", "地块编码", "XZQMC1", "地域名称", "承包方名称", "地块名称", "MJ_MU_QQ", "TXZQMC"])
    for row in cursor:
        txz_qmc = row[7]
        lb_bm = row[0]
        # dk_bm = row[1]
    ...
    for ys in x_list[so_id]:  # ys 默认为村名
        ...
        workbook_1.save('%s%s_2.xls' % (czpath, ys))
