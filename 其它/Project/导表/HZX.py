...
data_path = "E:/T赫章县台账数据.gdb/T无措施数据_确权_单部件_1严格管控类_大于五"  # 数据所在路径
...
def set_excel_title(ws, xz):
    ...
    ws.write_merge(0, 0, 0, 24, '赫章县%s耕地种植情况调查表（三）' % xz, style_1)
    ...
    ws.write_merge(2, 2, 6, 8, '2019种植品种', style_2)
    ws.write_merge(2, 2, 9, 19, '2020种植品种', style_2)
    ws.write_merge(2, 2, 20, 22, '其他', style_2)
    ws.write_merge(2, 4, 23, 23, '周边环境', style_2)
    ws.write_merge(2, 4, 24, 24, '备注', style_2)

    ws.write_merge(3, 4, 6, 6, '水稻', style_2)
    ws.write_merge(3, 4, 7, 7, '玉米', style_2)
    ws.write_merge(3, 4, 8, 8, '其他', style_2)
    ws.write_merge(3, 4, 9, 9, '水稻', style_2)
    ws.write_merge(3, 4, 10, 10, '玉米', style_2)
    ws.write_merge(3, 4, 11, 11, '小麦', style_2)
    ws.write_merge(3, 4, 12, 12, '薯类', style_2)
    ws.write_merge(3, 4, 13, 13, '豆类', style_2)
    ws.write_merge(3, 4, 14, 14, '油料', style_2)
    ws.write_merge(3, 4, 15, 15, '蔬菜', style_2)
    ws.write_merge(3, 4, 16, 16, '果树', style_2)
    ws.write_merge(3, 4, 17, 17, '茶叶', style_2)
    ws.write_merge(3, 4, 18, 18, '烤烟', style_2)
    ws.write_merge(3, 4, 19, 19, '中药材', style_2)
    ws.write_merge(3, 4, 20, 20, '休耕', style_2)
    ws.write_merge(3, 4, 21, 21, '有机肥', style_2)
    ws.write_merge(3, 4, 22, 22, '非耕地', style_2)
...
def su_total(data_path, output_path, so_id, index):
    ...
    cm_list = []  # 收集所有村的列表
    #  索引地理数据库数据
    cursor = arcpy.da.SearchCursor(data, ["BSBM", "DKBM", "XZQMC", "DKLB", "ZJRXM", "DKMC", "ECMJM2", "TXZQMC"])
    for row in cursor:
        txz_qmc = row[7]
        lb_bm = row[0]
        xz_qmc = row[7]+row[2]
        ...
        dk_mc = row[5]
        mjm = round(row[6], 1)
        list1 = [txz_qmc, lb_bm, xz_qmc, cb_nh_xm, dk_mc, mjm]
    ...
    for ys in x_list[so_id]:  # ys 默认为村名
        ...
        ws.write_merge(len(ys_name_list) + 5, len(ys_name_list) + 5, 4, 5,
                       xlwt.Formula('SUM(F6:F%s)' % (len(ys_name_list) + 5)), style_5)

        print("%s toatle:%s done" % (ys, len(ys_name_list)))
        workbook_1.save('%s/%s-3.xls' % (czpath, cz))
