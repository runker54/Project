...
data = "H:/大方县资料数据20200524/20200525资料数据/播bH:/大方县资料数据20200524/20200525资料数据/播州区数据/T播州区台账数据20200415（编号老）.gdb/成果数据"  # 数据路径
path = "C:/Users/Administrator/Desktop/CY"  # 保存位置
...
with arcpy.da.SearchCursor(data, ['LBBM', 'FXZQMC', 'TXZQMC', 'XZQMC', 'ZXLON', 'ZXLAT', 'ZLLB', 'CQCS',
    ...
    for row in cursor:
        ...
        list1 = [row[0], row[1], row[2], row[2] + row[3], row[4], row[5], row[6], row[7], row[8]]
...
for cm in xc_list:
    ...
    #                '原位钝化',
    #                '定向调控', '微生物修复', '植物提取', '退耕还林还草', '变更为非耕地', '少耕免耕休耕', '种植结构调整', '其他措施', '综合治理技术', '备注', '未完成',
    #                '已完成', '任务面积']
    # for i in range(len(table_title)):
    #     if i < 8:
    #         ws.write_merge(1, 2, i+1, i+1, table_title[i], style_1)
    #     else:
    #         ws.write(1,1)
    r = 3  # 开始行
    key_list = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'VIP', 'FH', '无']
    xz = cm_list[0][2]
    cz = cm_list[0][3]
    ...
    for data_sys in cm_list:
        ...
        data_data = [data_sys[0], data_sys[2], data_sys[3], data_sys[6], data_sys[4], data_sys[5], data_sys[12],
                     data_sys[17], data_sys[18], data_sys[20]]
        # ws.write_merge(0, 0, 0, 13, '遵义市播州区安全利用任务分解表',style_1)
        ws.write_merge(1, 2, 0, 0, '序号')
        ws.write_merge(1, 2, 1, 1, '地块编号')
        ws.write_merge(1, 2, 2, 2, '行政村')
        ws.write_merge(1, 2, 3, 3, '质量类别')
        ws.write_merge(1, 2, 4, 4, '中心经度')
        ws.write_merge(1, 2, 5, 5, '中心纬度')
        for b in range(len(data_data)):
            ws.write(r, b, data_data[b], style_4)
        ws.row(0).set_style(xlwt.easyxf('font:height 320;'))
        # ws.col(0).width = 256 * len(data_data[0])
        # ws.col(1).width = 256 * len(data_data[1]) * 3
        # ws.col(2).width = 256 * len(data_data[2]) * 3
        # ws.col(3).width = 256 * len(data_data[3]) * 3
        # ws.col(4).width = 256 * 10
        # ws.col(5).width = 256 * 10
        # ws.col(6).width = 256 * len(data_data[6]) * 3
        ws.write(r, 25, xlwt.Formula('SUM(H%s:X%s)' % (r + 1, r + 1)), style_4)
        ws.write(r, 26, xlwt.Formula('SUM(H%s:Y%s)' % (r + 1, r + 1)), style_4)
        r = r + 1

    workbook.save('%s/%s.xls' % (czpath, cm))
