...
def pingmianbuzhitu(*args):
    ...
    document.add_picture("%s" % tupia_dict.get(message[0].value), width=shared.Cm(21.5))
    document.add_page_break()
    section = document.add_section(start_type=WD_SECTION_START.CONTINUOUS)
...
sheet_path = r"G:\紫云县台账\ZYX20200909_dissolve.xls"  # 表格位置
pingmiantu_path = r"G:\紫云县台账\pictures"  # 平面图位置
# xianchangtu_path = r"C:\Users\65680\Desktop\复兴镇\复兴镇"  # 现场图位置
diaochabiao_path = r"G:\紫云县台账\ZYX_PDF"  # 调查表位置
...
for one_xz in xz_list:
    print(one_xz)
    try:
        os.makedirs(os.path.join(r'F:\ZYX', "%s" % one_xz))
    except:
        print("文件夹已创建")
    ...
    for one_cz in cz_list:
        if os.path.exists(r"F:\ZYX\%s\%s.docx" % (one_xz, one_cz)):
            print("文档已存在")
        else:
            ...
            document.save(r'F:\ZYX\%s\%s.docx' % (one_xz, one_cz))
            print(one_cz)
            time.sleep(3)
