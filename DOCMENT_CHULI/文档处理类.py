...
sheet_path = r"C:\Users\65680\Desktop\复兴镇.xls"  # 表格位置
pingmiantu_path = r"F:\pictures\复兴镇"  # 平面图位置
...
for one_row in range(1, work_sheet.nrows):  # 逐一写入各个地块情况及资料
    ...
    cz_list.append(message1[3].value)
    cz_list = list(set(cz_list))

print(xz_list)
time.sleep(5)
for one_xz in cz_list:
    document = Document()  # 创建空文档
    one_message_list = list(filter(lambda x: x[3].value == one_xz, message_list))
    time.sleep(5)
    ...
    doc_().save(r'C:\Users\65680\Desktop\DICK\%s.docx' % one_xz)
    print(one_xz)
    time.sleep(3)
