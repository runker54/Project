...
def diaochabiao(**args):
    ...
    section.page_width = page_w
    section.page_height = page_h
    document.add_picture("%s" % diaochabiao_dict.get(message[0].value), width=shared.Cm(19),height=shared.Cm(23))
    document.add_page_break()
    section = document.add_section(start_type=WD_SECTION_START.CONTINUOUS)
    section.orientation = WD_ORIENTATION.PORTRAIT  # 设置纵向（还原，为插入文档做准备）
    page_h, page_w = section.page_width, section.page_height
...
def dikuai_shuoming(*args):
    ...
    section.page_width = page_w
    section.page_height = page_h
    document.add_picture("%s" % shuoming_dict.get(message[0].value), width=shared.Cm(19),height=shared.Cm(25))
    document.add_page_break()
    section = document.add_section(start_type=WD_SECTION_START.CONTINUOUS)
    section.orientation = WD_ORIENTATION.PORTRAIT  # 设置纵向（还原，为插入文档做准备）
    page_h, page_w = section.page_width, section.page_height
...
sheet_path = r"F:\湄潭县台账\4湄潭县表格信息\MTX20200908_dissolve.xls"  # 表格位置
pingmiantu_path = r"F:\湄潭县台账\3湄潭县平面布置图"  # 平面图位置
# xianchangtu_path = r"C:\Users\65680\Desktop\高台镇\高台镇现场图片"  # 现场图位置
diaochabiao_path = r"F:\湄潭县台账\1湄潭县调查表"  # 调查表位置
...
for one_xz in xz_list:
    print(one_xz)
    try:
        os.makedirs(os.path.join(r'F:\MTX', "%s" % one_xz))
    except:
        print("文件夹已创建")
    ...
    for one_cz in cz_list:
        if os.path.exists(r"F:\MTX\%s\%s.docx" % (one_xz, one_cz)):
            print("文档已存在")
        else:
            ...
            document.save(r'F:\MTX\%s\%s.docx' % (one_xz, one_cz))
            del document
            print(one_cz)
            time.sleep(3)
