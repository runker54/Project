...
def pingmianbuzhitu(*args):
    ...
    document.add_picture("%s" % tupia_dict.get(message[0].value),  height=shared.Cm(24.3), width=shared.Cm(16.2))
    # document.add_page_break()
    # section = document.add_section(start_type=WD_SECTION_START.CONTINUOUS)
...
sheet_path = r"E:\台账\清镇市\清镇市导表\清镇市导表20200721.xls"  # 表格位置
pingmiantu_path = r"C:\Users\65680\Desktop\平面布置图"  # 平面图位置
# xianchangtu_path = r"G:\大方县台账\大方县现场图片"  # 现场图位置
diaochabiao_path = r"C:\Users\65680\Desktop\cut调查表"  # 调查表位置
...
print(xz_list)
time.sleep(5)
for one_xz in ['青龙山街道办事处']:
    print(one_xz)
    try:
        os.makedirs(os.path.join(r'C:\Users\65680\Desktop\文档', "%s" % one_xz))
    except:
        print("文件夹已创建")
    ...
    for one_cz in cz_list:
        if os.path.exists(r"C:\Users\65680\Desktop\文档\%s\%s.docx" % (one_xz, one_cz)):
            print("文档已存在")
        else:
            ...
            for message in two_message_list:
                ...
                try:
                    ...
                    print("无图片")
            document.save(r'C:\Users\65680\Desktop\文档\%s\%s.docx' % (one_xz, one_cz))
            print(one_cz)
            time.sleep(3)
