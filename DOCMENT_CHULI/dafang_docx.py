...
sheet_path = r"G:\大方县台账\DFX_20201001_dissolve.xls"  # 表格位置
pingmiantu_path = r"G:\大方县台账\大方县平面布置图旋转后"  # 平面图位置
xianchangtu_path = r"G:\大方县台账\大方县现场图片"  # 现场图位置
diaochabiao_path = r"G:\大方县台账\大方县调查表扫描裁剪"  # 调查表位置
...
print(xz_list)
time.sleep(5)
for one_xz in ['理化苗族彝族乡', '普底彝族苗族白族乡', '雨冲乡']:
    print(one_xz)
    try:
        os.makedirs(os.path.join(r'F:\DFX1', "%s" % one_xz))
    except:
        print("文件夹已创建")
    ...
    for one_cz in cz_list:
        if os.path.exists(r"F:\DFX1\%s\%s.docx" % (one_xz, one_cz)):
            print("文档已存在")
        else:
            ...
            for message in two_message_list:
                ...
                try:
                    ...
                    print("无图片")
            document.save(r'F:\DFX1\%s\%s.docx' % (one_xz, one_cz))
            print(one_cz)
            time.sleep(3)
