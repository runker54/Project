...
arcpy.env.workspace = r"C:\Users\65680\Desktop\玉屏县\大龙镇.gdb"

lbbm_list = []
with arcpy.da.SearchCursor("大龙镇", ["LBBM", "Shape_Area"]) as currsor:
    for row in currsor:
        if row[1] <= 120:
            lbbm_list.append(row[0])
lbbm_list = list(set(lbbm_list))
layers_1 = arcpy.MakeFeatureLayer_management("大龙镇", "testlyr")  # 创建总视图
# 个数统计
